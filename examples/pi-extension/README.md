...
When Pi exposes a session file through `ctx.sessionManager.getSessionFile()`,
the extension reads it before each tool call, extracts recent user/assistant
messages on a best-effort basis, and sends them as `chat_context` to Atryum.
LLM-as-judge rules then receive that chat context along with the tool call and
tool metadata.
