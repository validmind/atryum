...
- **<span style="font-variant: small-caps;">agent record</span>** — The agent record linked to this invocation — a local record manually registered or a record synced from ValidMind. Populated when the call's agent identity matches an **Agent IDs** entry on that record; empty otherwise. ([Connect agents](integrations/connect-agents.md#tag-invocations-to-agent-records), [Connect ValidMind](integrations/connect-validmind.md))
...
- A **ValidMind** connection with at least one model configuration available in your organization ([Connect ValidMind](integrations/connect-validmind.md))
- Or, at least one enabled **Local LLM Configuration** ([Configure LLM providers](integrations/configure-llm-providers.md))
