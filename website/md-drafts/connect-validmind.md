...
In addition to mapping tool invocations to agent records, allowing Atryum to use agent-scoped rules ([Rules](rules.md)), and powering invocation summarization ([Invocations](invocations.md)), the ValidMind integration also allows you to optionally use Atryum to evaluate tool invocations against each agent’s *constitution*:
