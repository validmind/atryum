...
In addition to mapping tool invocations to agent records, allowing Atryum to use agent-scoped rules ([Rules](../3_rules.md)), and powering invocation summarization ([Invocations](../2_invocations.md)), the ValidMind integration also allows you to use Atryum to evaluate tool invocations against each agent's *charter*:
...
- [x] Add a custom long text field to that record type for each agent's charter. ([Manage inventory fields](https://docs.validmind.ai/guide/inventory/manage-inventory-fields.html))
- [x] Create at least one record in that record type and fill in its charter field. ([Register records in the inventory](https://docs.validmind.ai/guide/inventory/register-records-in-inventory.html))
