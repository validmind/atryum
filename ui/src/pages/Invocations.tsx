...
const Header: React.FC<TextProps> = ({ children, ...props }) => (
  <Text
    fontSize="sm"
    fontWeight="semibold"
...
const Invocations: React.FC = () => {
    ...
    for (const agent of agentsData?.items ?? []) {
      ...
      for (const binding of agent.claude_managed_agents ?? []) {
        if (!map.has(binding.claude_agent_id)) {
          map.set(binding.claude_agent_id, {
            cuid: agent.cuid,
            name: agent.name,
          });
        }
  ...
  const resolvedSelectedId = useMemo(() => {
    ...
    return stillSelected ? selectedId : null;
  ...
  const selectedIndex = items.findIndex(
    (item) => item.invocation_id === resolvedSelectedId,
  );
  ...
  const handleNavigate = (direction: -1 | 1) => {
    ...
    const nextInvocation = items[selectedIndex + direction];
    if (nextInvocation) selectInvocation(nextInvocation);
  };
  ...
  return (
    <Box h="full" display="flex" flexDirection="column">
      ...
      <Modal
        isOpen={resolvedSelectedId !== null}
        onClose={handleCloseDetail}
        size="6xl"
        scrollBehavior="inside"
        isCentered>
        <ModalOverlay />
        <ModalContent maxH="85vh" data-testid="invocation-detail-modal">
          {detail && (
            <ModalHeader pr={12}>
              <HStack justify="space-between" gap={3} align="center">
                <Text fontFamily="mono" noOfLines={1} flex={1} minW={0}>
                ...
                <HStack gap={1} mr={4}>
                  {selectedIndex >= 0 && (
                    <Text
                      ...
                      color="text.subtle"
                      whiteSpace="nowrap"
                      data-testid="invocation-detail-position">
                  ...
                  <ButtonGroup>
                    <IconButton
                      ...
                      title="Previous invocation"
                      icon={<Icon as={ChevronUpIcon} boxSize={5} />}
                      isDisabled={!hasPrev}
                      onClick={() => handleNavigate(-1)}
                      data-testid="invocation-detail-prev-button"
                    />
                    <IconButton
                      size="sm"
                      variant="ghost"
                      aria-label="Next invocation"
                      title="Next invocation"
                      icon={<Icon as={ChevronDownIcon} boxSize={5} />}
                      isDisabled={!hasNext}
                      onClick={() => handleNavigate(1)}
                      data-testid="invocation-detail-next-button"
                    />
                  </ButtonGroup>
                </HStack>
              </HStack>
              ...
              <VStack align="stretch" gap={8}>
                  ...
                  <VStack align="start" gap={4} flex={1} minW={0}>
                    <HStack justify="space-between" align="start" w="full">
                      <HStack gap={2}>
                        ...
                        <Text>{formatDate(detail.submitted_at)}</Text>
                      </HStack>
                    ...
                    <VStack align="start" gap={1} w="full">
                      ...
                      {detail.approval?.reason && (
                        <Stack gap={1}>
                          ...
                          <Text>{detail.approval.reason}</Text>
                        </Stack>
                      )}
                    </VStack>
                    {detail.matched_rule_id && (
                      <Stack gap={1}>
                        <Header>Matched Rule:</Header>
                    ...
                    {(detail.agent_id ||
                      ...
                      <HStack gap={12}>
                        ...
                        {detail.agent_id && (
                          <Stack gap={1}>
                            <Header>Agent ID:</Header>
                            <Text fontFamily="mono">{detail.agent_id}</Text>
                          </Stack>
                        )}
                        {detail.user_id && (
                ...
                <Modal
                  ...
                  <ModalContent data-testid="create-rule-modal">
                    <ModalHeader>Create Rule From This Invocation</ModalHeader>
